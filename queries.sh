...
echo "$($PSQL "SELECT MAX(winner_goals) AS maxi FROM games")"

echo -e "\nNumber of games where the winning team scored more than two goals:"
echo "$($PSQL "SELECT COUNT(*) FROM games WHERE winner_goals > 2")"

echo -e "\nWinner of the 2018 tournament team name:"
echo "$($PSQL "SELECT name FROM teams WHERE team_id=89")"

echo -e "\nList of teams who played in the 2014 'Eighth-Final' round:"
echo "$($PSQL "SELECT name FROM games AS g FULL JOIN teams AS t ON g.opponent_id=t.team_id OR g.winner_id=t.team_id WHERE year=2014  AND round='Eighth-Final' ORDER BY name")"
